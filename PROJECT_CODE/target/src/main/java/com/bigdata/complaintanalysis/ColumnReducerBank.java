...
public class ColumnReducerBank {

	public void reduceColumn (String args1, String args2) throws Exception {
		if (args1 == null && args2 == null) {
			System.err.println("ColumnReducerBank::main(): Don't have input and output paths {Arguments: [Input CSV File] [Output Sequence Directory] }");
		}

		Map<String, Integer> fieldIndex = new HashMap<String, Integer>();
		String inputFile = args1;
		String outputDir = args2;
		String inputFields;
		Scanner input = new Scanner(System.in);
		String[] fields = {"Company Response?", "Complaint ID", "Product", "Company"};
		...
		FileWriter writer = new FileWriter(outputDir + "BankSortedData.csv");
		...
		while (true) {
			...
			if (columns.length != 14) {
				System.out.println("ColumnReducerBank::main(): Invalid Line");
			}
			String product = columns[fieldIndex.get("Product")];
			String complaintID = columns[fieldIndex.get("Complaint ID")];
			String response = columns[fieldIndex.get("Company Response?")];
			String company = columns[fieldIndex.get("Company")];
			...
			catch (IOException e) {System.out.println("ColumnReducerBank::main(): Error writing File"); }
			count ++;
		}
		reader.close();
		writer.close();
		System.out.println("ColumnReducer::main() Reduced " + count + " files to required number of columns ");
	}
}